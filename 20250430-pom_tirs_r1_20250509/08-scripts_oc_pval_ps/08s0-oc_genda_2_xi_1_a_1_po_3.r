# setwd("C:/Users/.../")
rm(list = ls())
i.gen.da <- 2    # 1 for same covariate, 2 for different
...
# for(i.gen.da in 1:2){
    ...
    # for(i.a in 1:length(a)){
      ...
      for(i.trt.eff in 1:length(oc.trt.eff)){
        ...
        for(i.sim in 1:n.sim){
          ...
          ### get combined data
          da.comb <- rbind(cbind(da.trt$y, da.trt$X, 1),
                           cbind(da.ctr$y, da.ctr$X, 0))
          n.X <- ncol(da.trt$X)
          colnames(da.comb) <- c("y", paste("X", 1:n.X, sep = ""), "Z")
          da.comb <- as.data.frame(da.comb)

          ### get formula
          fml.ps <- paste("Z~", paste("X", 1:n.X, sep = "", collapse = "+"),
                       sep = "")
          fml.ps <- as.formula(fml.ps)
          fml.lm <- as.formula("y ~ Z")

          ### get ps ipw est (ATT)
          w.att <- weightit(fml.ps, data = da.comb,
                            method = "glm", estimand = "ATT")
          f.ps.weight <- lm_weightit(fml.lm, data = da.comb, weightit = w.att)
          sm.ps.weight <- summary(f.ps.weight)

          ret.ps.weight <- list(trt.pv = sm.ps.weight$coefficients[2, 4],
                                trt.eff = sm.ps.weight$coefficients[2, 1],
                                trt.ci = confint(f.ps.weight)[2,])

          ### get ps matching 1:1 est (ATT)
          w.match <- matchit(fml.ps, data = da.comb, ratio = 1,
                             method = "nearest", distance = "glm",
                             link = "logit", estimand = "ATT")
          da.match <- match_data(w.match)
          f.ps.match <- lm(fml.lm, data = da.match, weights = weights)
          sm.ps.match <- avg_comparisons(f.ps.match,
                                         variables = "Z",
                                         newdata = subset(Z == 1))

          ret.ps.match <- list(trt.pv = sm.ps.match$p.value,
                               trt.eff = sm.ps.match$estimate,
                               trt.ci = c(sm.ps.match$conf.low,
                                          sm.ps.match$conf.high))
          ...
          tmp.delta <- c(ret.po.vcov$trt.eff,
                         ...
                         ret.ps.weight$trt.eff,
                         ret.ps.match$trt.eff)
        ...
        alpha.level.adj <- rep(alpha.level, ret.n.var)
  ...
  colnames(ret.pval) <- tmp.name.d
  colnames(ret.trt.eff) <- tmp.name.d


  fn <- paste("./results_oc_pval_ps/08s0-oc_",
