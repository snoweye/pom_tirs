    ...
    for(i.a in 1){
      for(i.npo in 1:npo.all){
        for(i.n.trt in 1:n.trt.all){
          ...
          tmp.mse <- cbind(rep(i.genda, nrow(ret.mse)),
                           ret.mse,
                           n.simu.po,
                           n.trt)
          da.org <- rbind(da.org, tmp.mse)
        }
      }
    }
  }
}
colnames(da.org) <- c("genda", colnames(ret.mse), "npo", "n.trt")
...
g1 <- ggplot(data = da.g1,
 ...
 png(paste(pn.d, "./paper_supp_plots_v4/Rplot_ntrt_cont_s0_comb_mse_mcvar.png", sep = ""),
